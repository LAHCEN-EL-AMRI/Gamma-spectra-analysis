...
public class treatmentLog extends JPanel {	
	
	       static XYPlot plot;
           XYLineAndShapeRenderer renderer;
    static ArrayList<String> DomainAxisPeakCmpr;
    static ArrayList<String> RangAxisPeakCmpr;
    static ArrayList<String> BckgrdAxisPeakCmpr;
    static ArrayList<String> FwhmAxisPeakCmpr;
    static ArrayList<String> ErrorAxisPeakCmpr;
    static ArrayList<String> EnergyAxisPeakCmpr;
    static ArrayList<String> AreayAxisPeakCmpr;   
    static ArrayList<String> ForIdentification;
	...
	public treatmentLog() throws IOException {
		Mainn.forRaport="logTreatmnt";
		DomainAxisPeakCmpr        = new ArrayList<String>(); //pour zoomer chaque pic
        RangAxisPeakCmpr          = new ArrayList<String>(); //pour zoomer chaque pic
        BckgrdAxisPeakCmpr        = new ArrayList<String>(); //pour zoomer chaque pic
        FwhmAxisPeakCmpr          = new ArrayList<String>(); //pour zoomer chaque pic
        ErrorAxisPeakCmpr          = new ArrayList<String>(); //pour zoomer chaque pic
        EnergyAxisPeakCmpr          = new ArrayList<String>(); //pour zoomer chaque pic
        AreayAxisPeakCmpr          = new ArrayList<String>(); //pour zoomer chaque pic
        ForIdentification         = new ArrayList<String>(); //pour identifier les pics
        DomainAxisPeakCmpr.clear();
        RangAxisPeakCmpr.clear();
        BckgrdAxisPeakCmpr.clear();
        FwhmAxisPeakCmpr.clear();
        ErrorAxisPeakCmpr.clear();
        EnergyAxisPeakCmpr.clear();
        AreayAxisPeakCmpr.clear();
        ForIdentification.clear();
	...
	private XYDataset createDataset() throws IOException {
      ...
      //----------------------------------------------------------
		...
		XYSeries seriesDernier = new XYSeries("dernier",false,true);
          ...
          if(Mainn.rdbtnmntmView_5.isSelected()){if(x>0) {series2.add(je, Math.log(x));}}		
          je=je+1;
	  }
        ...
        for(j4=0;j4<mariscoti.ChannelLissee.size();j4++){
        		           ...
        		           if(Nsmzc<0){
        			          ...
        			          if(l>=lmin && l<lmax && t>=tlim){       			
        				            ...
        				            float NDpZ = (float) Math.log(NDm); //Pour zommer chaque pic
        				            String NDpZZ = Float.toString(NDpZ); //Pour zommer chaque pic
        				            DomainAxisPeakCmpr.add(Xs_m);//Pour zommer chaque pic
        				            RangAxisPeakCmpr.add(NDpZZ);//Pour zommer chaque pic
        	...
        	for(ia=0;ia<xs_m.size()-1;ia++){
				...
				//___________________________________multipl-pic________________________________________________________________
				                            ...
				                            if(jcc==0 && Pg>7*ecartc){//3*ecart pour éviter la premier dessigne en graphe				
				                                      ...
				                                      for(im=0;im<yd.size()-1;im++){
				                                          ...
				                                          if(Pg-Pg_1>(int) k*ecartg+tallg+5 && xdm1>= Pg-withg-tallg-5 && xdm1<Pg-withg-tallg){nmg=nmg+1; Bmg=Bmg+ydm1;Eg= (int) (Pg-withg-tallg); if(Mainn.rdbtnmntmView_2.isSelected()){if(ydm1>0) {series4.add(xdm1, Math.log(ydm1));}}}
				                                          else if(Pg-Pg_1>(int) k*ecartg+tallg+3 && xdm1>=Pg-withg-tallg-3 && xdm1<Pg-withg-tallg){nmg=nmg+1; Bmg=Bmg+ydm1;Eg=(int) (Pg-withg-tallg); if(Mainn.rdbtnmntmView_2.isSelected()){if(ydm1>0) {series4.add(xdm1, Math.log(ydm1));}}}
				                                          else if(Pg-Pg_1>=(int) (k*ecartg+tallg) && xdm1>=Pg-withg-tallg-1 && xdm1<Pg-withg-tallg){nmg=nmg+1; Bmg=Bmg+ydm1;Eg=(int) (Pg-withg-tallg);if(Mainn.rdbtnmntmView_2.isSelected()){if(ydm1>0) {series4.add(xdm1, Math.log(ydm1));}}}
				
				                                          if((int)(Pg-withg-tallg)<=xdm1 && xdm1<=(int)(Pd+withd+0.5)){Nm=Nm+1;areatm=areatm+ydm1;if(Mainn.rdbtnmntmView_1.isSelected()){if(ydm1>0) {series6.add(xdm1, Math.log(ydm1));}}} 
				
				                                          if(Pd1-Pd>(int) k*ecartd+tallPd1+5 && xdm1>Pd+withd && xdm1<=Pd+withd+5){nmd=nmd+1; Bmd=Bmd+ydm1;Ed=(int) (Pd+withd);if(Mainn.rdbtnmntmView_2.isSelected()){if(ydm1>0) {series5.add(xdm1, Math.log(ydm1));}}}
				                                          else if(Pd1-Pd>(int) k*ecartd+tallPd1+3 && xdm1>Pd+withd && xdm1<=Pd+withd+3){nmd=nmd+1; Bmd=Bmd+ydm1;Ed=(int) (Pd+withd);if(Mainn.rdbtnmntmView_2.isSelected()){if(ydm1>0) {series5.add(xdm1, Math.log(ydm1));}}}
				                                          else if(Pd1-Pd>=(int) k*ecartd+tallPd1 && xdm1>Pd+withd && xdm1<=Pd+withd+1){nmd=nmd+1; Bmd=Bmd+ydm1;Ed=(int) (Pd+withd);if(Mainn.rdbtnmntmView_2.isSelected()){if(ydm1>0) {series5.add(xdm1, Math.log(ydm1));}}}
				                                          } 
				                                      ...
				                                      for(ima=0;ima<yd.size()-1;ima++){
				                                          ...
				                                          if(Mainn.rdbtnmntmView_3.isSelected()){if(drt>0) {series7.add(xdm1a, Math.log(drt));}}
				                                          } 
				                                        }
				                                      int i1m=0,hn=0;
				                                      ...
				                                      for(i1m=0;i1m<yd.size()-1;i1m++){
				                                          if(i1m<=ia && i1m>=jg){ 
	        		                                             ...
	        		                                             for(ig=(int) (Pi-(withi-(int)(talli-0.5)+1));ig<=(int) Pi+(withi-1);ig++){            
	        		                                            	              ...
	        		                                            	              if((int)Pi-(withi-talli+1)>(int)(withi-talli+1)+6){//pour eviter le le probleme de premier pic
	        			                                                                    ...
	        			                                                                    if(Mainn.rdbtnmntmView_4.isSelected()){
	        			                                                                    	if(clor<=1000){
	        			                                                                    		if(gauss>0) {	Srie.get(clor).add(Dge, Math.log(gauss));	}
	        			                                                                         }else{ if(gauss>0) {seriesDernier.add(Dge, Math.log(gauss));}}
	        			                                                                    }
	        		                                             ...
	        		                                             ll=ll+1;
	       				                                         ...
	       				                                         ForIdentification.add(tr.format(ll)+"M"+"    "+df0.format(Pi)+espaceId+df0.format(enrgi)+espaceId+df0.format(ecarti)+espaceId+df0.format(area_net_gaus)+espaceId+df0.format(drtgBck)+espaceId+df0.format( eror_area_net));
	       				                                      
	       				                                         
	       				                                         BckgrdAxisPeakCmpr.add(Float.toString((float) Math.log((MBg+MBd)/2)));//pour zommer chaque pic
	        		                                             FwhmAxisPeakCmpr.add(Float.toString(ecarti));//pour zommer chaque pic
	        		                                             EnergyAxisPeakCmpr.add(Float.toString(enrgi));
	        		                                             AreayAxisPeakCmpr.add(Float.toString(area_net_gaus));
	        		                                             ErrorAxisPeakCmpr.add(Float.toString(eror_area_net));
				...
				for(is=0;is<yd.size()-1;is++){  
        		          ...
        		          if(Xa-Xa_1>(int) k*ecartj+tallj+5 && xd1>=Xa-withj-tallj-5 && xd1<Xa-withj-tallj){ng=ng+1; Bg=Bg+yd1; if(Mainn.rdbtnmntmView_2.isSelected()){if(yd1>0) {series4.add(xd1, Math.log(yd1));}}}
        		          else if(Xa-Xa_1>(int) k*ecartj+tallj+3 && xd1>=Xa-withj-tallj-3 && xd1<Xa-withj-tallj){ng=ng+1; Bg=Bg+yd1; if(Mainn.rdbtnmntmView_2.isSelected()){if(yd1>0) {series4.add(xd1,Math.log( yd1));}}}
        		          else if(Xa-Xa_1>=(int) k*ecartj+tallj && xd1>=Xa-withj-tallj-1 && xd1<Xa-withj-tallj){ng=ng+1; Bg=Bg+yd1;if(Mainn.rdbtnmntmView_2.isSelected()){if(yd1>0) {series4.add(xd1, Math.log(yd1));}}}
				
        		          if(Xa-withj-tallj<=xd1 && xd1<=Xa+withj){N=N+1;areat=areat+yd1;if(Mainn.rdbtnmntmView.isSelected()){ if(yd1>0) {   series3.add(xd1, Math.log(yd1));  } }} 
				
        		          if(Xa1-Xa>(int) k*ecartj+tallj1+5 && xd1>Xa+withj && xd1<=Xa+withj+5){nd=nd+1; Bd=Bd+yd1;if(Mainn.rdbtnmntmView_2.isSelected()){if(yd1>0) {series5.add(xd1, Math.log(yd1));}}}
        		          else if(Xa1-Xa>(int) k*ecartj+tallj1+3 && xd1>Xa+withj && xd1<=Xa+withj+3){nd=nd+1; Bd=Bd+yd1;if(Mainn.rdbtnmntmView_2.isSelected()){if(yd1>0) {series5.add(xd1, Math.log(yd1));}}}
        		          else if(Xa1-Xa>=(int) k*ecartj+tallj1 && xd1>Xa+withj && xd1<=Xa+withj+1){nd=nd+1; Bd=Bd+yd1;if(Mainn.rdbtnmntmView_2.isSelected()){if(yd1>0) {series5.add(xd1, Math.log(yd1));}}}
				}
				...
				ForIdentification.add(tr.format(ll)+"S "+"   "+df0.format(Xa)+espaceId+df0.format(enrgj)+espaceId+df0.format(ecartj)+espaceId+df0.format(Area_net)+espaceId+df0.format(backgrounds)+espaceId+df0.format(eror));
				
				 BckgrdAxisPeakCmpr.add( Float.toString((float) Math.log(Bg/ng+Bd/nd)/2));//pour zommer chaque pic
				 FwhmAxisPeakCmpr.add(Float.toString(ecartj));//pour zommer chaque pic
				 EnergyAxisPeakCmpr.add(Float.toString(enrgj));
				 AreayAxisPeakCmpr.add(Float.toString(Area_net));
                 ErrorAxisPeakCmpr.add(Float.toString(eror));
